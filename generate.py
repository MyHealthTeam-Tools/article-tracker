...
.site-trigger{display:flex;align-items:center;gap:6px;padding:7px 10px;border:1px solid #e2e8f0;border-radius:7px;background:#fff;font-size:12px;color:#475569;cursor:pointer;white-space:nowrap;min-width:150px;justify-content:space-between;font-family:inherit}
.site-trigger.active{border-color:#a5f3fc;background:#ecfeff;color:#0e7490;font-weight:600}
.site-dropdown{position:absolute;top:calc(100% + 4px);left:0;background:#fff;border:1px solid #e2e8f0;border-radius:9px;box-shadow:0 8px 24px rgba(0,0,0,.15);z-index:9999;min-width:260px;padding:6px 0;display:none}
.site-dropdown.open{display:block}
.site-search{width:100%;padding:7px 12px;border:none;border-bottom:1px solid #f1f5f9;font-size:12px;color:#1e293b;outline:none;font-family:inherit}
.site-list{max-height:280px;overflow-y:auto}
.site-option{display:flex;flex-direction:row;align-items:center;gap:10px;padding:7px 12px;cursor:pointer;font-size:13px;color:#475569;user-select:none;width:100%}
.site-option:hover{background:#f1f5f9}
.site-option input[type=checkbox]{width:15px;height:15px;cursor:pointer;accent-color:#0891b2;flex-shrink:0}
/* Multi-select type */
...
.type-option{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:10px;padding:8px 12px;cursor:pointer;font-size:13px;color:#475569;user-select:none;width:100%;box-sizing:border-box}
...
<div class="filters">
  ...
  <div class="site-wrap" id="site-wrap">
    <button class="site-trigger" id="site-trigger" onclick="toggleSiteDropdown(event)">
      <span id="site-trigger-label">All Sites</span>
      <span style="font-size:10px;color:#94a3b8;margin-left:4px">&#9660;</span>
    </button>
    <div class="site-dropdown" id="site-dropdown">
      <input class="site-search" id="site-search-input" type="text" placeholder="Search sites..." oninput="filterSiteOptions()"/>
...
let state={tab:"added",month:CUR_MONTH,sites:new Set(),types:new Set(),q:"",groupByType:false,liveOnly:false};
...
// Site dropdown
const allSites=Array.from(new Set([...ADDED,...REFRESHED,...REMOVED].map(r=>r[S]).filter(Boolean))).sort();
...
function buildSiteList(filter){
  ...
  allDiv.style.cssText="display:grid;grid-template-columns:auto 1fr;align-items:center;gap:10px;padding:8px 12px;cursor:pointer;box-sizing:border-box;width:100%";
  allDiv.onmouseenter=e=>e.currentTarget.style.background="#f1f5f9";
  allDiv.onmouseleave=e=>e.currentTarget.style.background="";
  const allCb=document.createElement("input");allCb.type="checkbox";
  allCb.style.cssText="width:15px;height:15px;cursor:pointer;accent-color:#0891b2;flex-shrink:0";
  allCb.checked=state.sites.size===0;
  const allLbl=document.createElement("span");allLbl.textContent="All Sites";allLbl.style.cssText="font-weight:600;font-size:13px;color:#1e293b";
  allDiv.append(allCb,allLbl);
  allDiv.onclick=()=>{state.sites=new Set();updateSiteTrigger();buildSiteList(document.getElementById("site-search-input").value);render();};
  list.appendChild(allDiv);
  const hr=document.createElement("div");hr.style.cssText="height:1px;background:#f1f5f9;margin:4px 0";list.appendChild(hr);
  const q=(filter||"").toLowerCase();
  const filtered=q?allSites.filter(s=>s.toLowerCase().includes(q)):allSites;
  filtered.forEach(site=>{
    const d=document.createElement("div");
    d.style.cssText="display:grid;grid-template-columns:auto 1fr;align-items:center;gap:10px;padding:8px 12px;cursor:pointer;box-sizing:border-box;width:100%";
    d.onmouseenter=e=>e.currentTarget.style.background="#f1f5f9";
    d.onmouseleave=e=>e.currentTarget.style.background="";
    const cb=document.createElement("input");cb.type="checkbox";
    cb.style.cssText="width:15px;height:15px;cursor:pointer;accent-color:#0891b2;flex-shrink:0";
    cb.checked=state.sites.has(site);
    cb.onclick=e=>e.stopPropagation();
    cb.onchange=()=>{if(cb.checked)state.sites.add(site);else state.sites.delete(site);updateSiteTrigger();render();};
    const c=sc(site);
    const badge=document.createElement("span");
    badge.style.cssText=`display:inline-block;font-size:11px;font-weight:700;padding:2px 7px;border-radius:5px;border:1px solid;background:${c}15;color:${c};border-color:${c}30`;
    badge.textContent=site;
    d.append(cb,badge);
    d.onclick=()=>{cb.checked=!cb.checked;if(cb.checked)state.sites.add(site);else state.sites.delete(site);updateSiteTrigger();render();};
    list.appendChild(d);
  });
  if(filtered.length===0){const empty=document.createElement("div");empty.style.cssText="padding:12px;color:#94a3b8;font-size:12px;text-align:center";empty.textContent="No sites found";list.appendChild(empty);}
}
function filterSiteOptions(){buildSiteList(document.getElementById("site-search-input").value);}
function toggleSiteDropdown(e){
  e.stopPropagation();
  const dd=document.getElementById("site-dropdown");
  dd.classList.toggle("open");
  if(dd.classList.contains("open")){buildSiteList("");setTimeout(()=>document.getElementById("site-search-input").focus(),50);}
}
function updateSiteTrigger(){
  const tr=document.getElementById("site-trigger");
  const lb=document.getElementById("site-trigger-label");
  if(state.sites.size===0){tr.className="site-trigger";lb.textContent="All Sites";}
  else if(state.sites.size===1){tr.className="site-trigger active";lb.textContent=[...state.sites][0];}
  else{tr.className="site-trigger active";lb.innerHTML=`Sites <span style="background:#0891b2;color:#fff;font-size:10px;font-weight:700;padding:1px 6px;border-radius:10px;margin-left:2px">${state.sites.size}</span>`;}
}
document.addEventListener("click",e=>{
  if(!document.getElementById("site-wrap").contains(e.target))document.getElementById("site-dropdown").classList.remove("open");
  if(!document.getElementById("type-wrap").contains(e.target))document.getElementById("type-dropdown").classList.remove("open");
});
...
function buildTypeDropdown(availTypes,countMap){
  ...
  allDiv.style.cssText="display:grid;grid-template-columns:auto 1fr;align-items:center;gap:10px;padding:8px 12px;cursor:pointer;width:100%;box-sizing:border-box";
  allCb.style.cssText="width:15px;height:15px;cursor:pointer;accent-color:#6366f1";
  ...
  availTypes.forEach(type=>{
    ...
    d.style.cssText="display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:10px;padding:8px 12px;cursor:pointer;user-select:none;width:100%;box-sizing:border-box";
    cb.style.cssText="width:15px;height:15px;cursor:pointer;accent-color:#6366f1;flex-shrink:0";
    pill.style.cssText="font-size:11px;font-weight:600;color:#94a3b8;background:#f1f5f9;padding:1px 7px;border-radius:10px;text-align:right";
...
// Filters
function siteF(r){return state.sites.size===0||state.sites.has(r[S]);}
...
function clearAll(){state.q="";state.sites=new Set();state.types=new Set();state.liveOnly=false;document.getElementById("search").value="";document.getElementById("site-search-input").value="";updateSiteTrigger();updateTypeTrigger();buildSiteList("");render();}
...
function render(){
  ...
  // Clear btn
  document.getElementById("clear-btn").style.display=(state.q||state.sites.size>0||state.types.size>0||state.liveOnly)?"":"none";
